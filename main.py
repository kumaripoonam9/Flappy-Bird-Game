...
# sounds
point = pygame.mixer.Sound("sounds/sounds_sfx_point.wav")
hit = pygame.mixer.Sound("sounds/sounds_sfx_hit.wav")
...
class Game:
    ...
    def isCollide(self):
        for i in range(0, 7):
            if(self.birdX >= self.pipesX[i] and
                self.birdX <= (self.pipesX[i]+pipe.get_width()) and
                ((self.birdY+bird.get_height()-15) >= self.lowerPipeY[i] or
